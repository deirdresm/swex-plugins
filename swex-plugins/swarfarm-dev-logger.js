...
const SWARFARM_DEV_URL = 'http://10.243.243.10/api/v2/';
const pluginName = 'SwarfarmDevLogger';
  ...
  if (apiKeyIsValid(token)) {
    ...
    if (token) {
      proxy.log({
        ...
        source: 'plugin',
        name: pluginName,
        message: 'Invalid API key. Copy/paste it from SWARFARM-DEV profile settings. Logging without API key until it is updated.'
      });
    }
  }
...
const getApiCommands = () => {
  ...
  proxy.log({
    ...
    source: 'plugin',
    name: pluginName,
    message: 'Retrieving list of accepted log types from SWARFARM-DEV...'
  });
...
const uploadProfile = (req, resp) => {
  ...
  if (command === 'HubUserLogin') {
    ...
    proxy.log({
      ...
      source: 'plugin',
      name: pluginName,
      message: 'Uploading profile to SWARFARM-DEV...'
    });
    ...
    request.post('profiles/upload/', req_options, (error, response, body) => {
      ...
      if (response.statusCode === 200) {
        proxy.log({
          ...
          source: 'plugin',
          name: pluginName,
          message: 'SWARFARM-DEV profile successfully uploaded - awaiting import queue.'
        });
            ...
            (resultError, resultResponse, resultBody) => {
              if (resultBody && resultBody.status === 'SUCCESS') {
                ...
                proxy.log({
                  ...
                  source: 'plugin',
                  name: pluginName,
                  message: 'SWARFARM-DEV profile import complete!'
                });
              }
            }
          );
        ...
        proxy.log({
          ...
          source: 'plugin',
          name: pluginName,
          message: `There were errors importing your SWARFARM-DEV profile: ${JSON.stringify(body)}`
        });
        ...
        proxy.log({
          ...
          source: 'plugin',
          name: pluginName,
          message: 'Unable to authenticate to SWARFARM-DEV. Check your API key in settings.'
        });
        ...
        proxy.log({
          ...
          source: 'plugin',
          name: pluginName,
          message: `${JSON.stringify(body)}. You must manually upload your profile on SWARFARM-DEV to resolve this.`
        });
      }
    });
  }
...
module.exports = {
  ...
  defaultConfigDetails: {
    profileSync: { label: 'Automatically upload profile to SWARFARM-DEV' },
    apiKey: { label: 'SWARFARM-DEV API key', type: 'textarea' }
  },
  pluginName,
  pluginDescription: 'Syncs your SWARFARM-DEV profile automatically and logs data for your account.',
  init(proxyInstance, config) {
    if (config.Config.Plugins[pluginName].enabled) {
      proxy = proxyInstance;
      request = requestLib.defaults({ baseUrl: SWARFARM_DEV_URL });
      ...
      if (!config.Config.Plugins[pluginName].apiKey) {
        if (config.Config.Plugins[pluginName].profileSync) {
          ...
          proxy.log({
            ...
            name: pluginName,
            message:
              'An API key is required for SWARFARM-DEV profile sync and recommended for data logging. Copy it into settings from your SWARFARM Edit Profile page to enable this feature.'
          });
        } else {
          ...
          proxy.log({
            ...
            name: pluginName,
            message:
              'You have not configured a SWARFARM-DEV API key in Settings. Logs will attempt to be associated to your SWARFARM-DEV account via in-game account ID, but this is not guaranteed to work.'
          });
        }
      }
      ...
      try {
        ...
        proxy.log({
          ...
          source: 'plugin',
          name: pluginName,
          message: 'Unable to retrieve accepted log types. SWARFARM-DEV logging is disabled.'
        });
      }
      ...
      proxy.on('apiCommand', (req, resp) => {
        ...
        try {
          ...
          proxy.log({
            ...
            source: 'plugin',
            name: pluginName,
            message: 'Error submitting log event to SWARFARM-DEV.'
          });
        }
